...
etc.  Security scanners may find individual files suspicious.  For example, the
following archive files of the ${/glossary/gcc:/term} Go language test suite
contain oversized files and deeply nested file system paths:
...
install the following addional packages:
...
This section explains how to set up a dockerized solution to use the package.
It allows using the package in any operating system supporting Docker.  The
Docker container presented below permits to
...
3. Extract the package archive as non-root user
   ...
   path to the directory where you want the unpacked the archive:
...
.. topic:: Running tests on your hardware board
    ...
    program or script must execute a test on your hardware.  It will be give
    the path to an executable and it is expected to return the console output
    of the board during the test execution as standard out.
