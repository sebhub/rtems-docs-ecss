...
RTEMS is designed as a reusable software product which can be utilized by
...
* Software Configuration File (SCF)
  ...
  used to verify the authenticiy and integrity of the QDP, see
  :ref:`InventorySCI`.
...
* Sofware Release Document (SRelD)
...
It is recommend to use the following optimization flags to compile C code of
...
option.  The rationale for using ``-O2`` is that this optimization level is
commonly used.
...
The application may use other optimization flags if necessary.  However, the
...
Using ``pkg-config`` to query the tool flags of the BSP avoids having hard
coded flags in the application build system.  It is recommended to place the
linker flags provided by ``pkg-config`` to the end of the linker command line
used to link the application executable.  See also :ref:`Examples`.
...
uninitialized read-write memory area to support the statically allocated RTEMS
tasks:
...
``.rtemsstack`` section is not zero initialized.  It is not initialized at all
...
Customize the inter-processor interrupt number
----------------------------------------------
