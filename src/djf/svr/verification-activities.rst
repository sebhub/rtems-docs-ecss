...
A automatically generated code files are treated as if there were hand written:
    ...
    * This means, the automatically produced code is not generated a new
      every time the operating system is build. Instead, the code is generated
      once, reviewed, committed to the other source code. The operating system
      is then build again and again from that once checked-in source code
...
for each requirement the associated desgin components.
...
* **Review Date:** 2021-11-26, 021-11-10,
...
4.  There is no |RB|. Instead, the |SP| is used here as substitute to
    at least check consitency against the |SP| somewhere.
    ...
    Hence testing is for sure feasible.
      ...
      a) |SRS| ${doc-ts-srs:/cite} section *5.2 Performance requirements* states:
         *There are no performance requirements*. Performance measurements
         and benchmarks are planed to be done only using the official
         externally visible API.

      b) There are no computational invariants or temporal properties
         mentioned in the design. All tests where so far possible without
      ...
      c) The design neither :term:`RTEMS` have provisions for fault injection.
         Tests using fault injection where never necessary nor are such tests
         planed. The errors typically appearing in operating systems
...
10. The :term:`SDD` contains elements which are not
    ...
    like ``Barrier Handler``,  ``Basic Types``, ``Bootcard``, etc. These
    are apparently needed to realize the components and functions
    defined in the :term:`ICD` and :term:`SRS`.
    ...
    archives 100% code coverage -- i.e. there are no unnecessary elements
   ...
   **Verification:** As random sample for 2020-12-12 -- 22 the partition
    ...
    **Comments:**
      ...
      * Specification items like ``spec/rtems/sem/req/timeout.yml``
        deligate part of their work to other specification items
        ...
        it calls ``spec/score/tq/req/timeout-mrsp`` etc.
...
flaws leading to catastrophic or unexpected behavior:
    ...
    * The static analyzer *Coverity* is used by the public :term:`RTEMS`
      ...
      devision by zero, automatic type casts with possible unwanted
      effects and other possible run-time errors such as the use of unsafe
      string copy functions or the use of wrong conversion specificators
    ...
    * The static analyzer *CLANG* is used by this :term:`ESA` qualification
      ...
      using several distinct tools decreases the likely hood for
...
7. Is normal termination (i.e. the test end criteria defined in the unit test
   plan) achieved?
...
8. Is abnormal termination of testing process (e.g. incorrect major fault,
   out of time) reported?
...
Collection and analysis of metrics as defined by :term:`SPAP` ${/ref/legacy/spap-r6:/cite}
    The metrics are reported and justified in an dedicated chapter in
    the |SPAMR|.
...
project and on the mailing list through out the last four months. Currently,
