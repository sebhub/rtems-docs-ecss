...
the Design Justification File (DJF) as required by ECSS-E-ST-40C
${/ref/ecss/e-st-40c:/cite} and ECSS-Q-ST-80C Rev.1 ${/ref/ecss/q-st-80c-r1:/cite}.  The
...
recommended to get started by reading the Software Configuration File
${/pkg/deployment/doc-package-manual:/cite}.  For using RTEMS in general, please refer to the RTEMS User
Manual ${/ref/rtems/user:/cite} and the RTEMS Classic API Guide
${/ref/rtems/c-user:/cite}.
...
register level.  This makes mocking difficult.  The inputs to units are no only
...
operation system which is developed from scratch it would make sense to write a
...
* `#4270: A failing task extension produces zombi objects and resource leaks <https://gitlab.rtems.org/rtems/rtos/rtems/-/issues/4270>`_
...
* `#4390: Make zero size allocation result consistent accross directives <https://gitlab.rtems.org/rtems/rtos/rtems/-/issues/4390>`_
...
* `#4412: Unexpected rtems_task_restart() behaviour if called from within interrrupt context <https://gitlab.rtems.org/rtems/rtos/rtems/-/issues/4412>`_
...
As an example how through the specification and validation is please have a
look at the action requirement item for the :c:func:`rtems_task_set_scheduler`
directive:
